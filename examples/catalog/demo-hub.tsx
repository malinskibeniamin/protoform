...
export function DemoHub({ category }: { category: DemoHubCategory }) {
  ...
  const [selectedSlug, setSelectedSlug] = useState(() => initialDemoSlug(demos));
  const selectedDemo = demos.find((demo) => demo.slug === selectedSlug) ?? demos[0];
  ...
  if (!selectedDemo) {
    return (
      <Alert variant="destructive">
        ...
        <AlertDescription>This documentation hub has no registered demos.</AlertDescription>
      </Alert>
    );
  }
    ...
    <div className="not-prose space-y-8 text-foreground">
      <Field>
        ...
        <Select onValueChange={handleDemoChange} value={selectedDemo.slug}>
          <SelectTrigger aria-label="Choose a demo" className="w-full" id={`${category}-demo`} size="lg">
      ...
      <section aria-labelledby={`${category}-demo-title`} className="space-y-6">
        <div className="space-y-3">
          ...
          <h2 className="font-semibold text-2xl tracking-tight" id={`${category}-demo-title`}>
            {selectedDemo.title}
          </h2>
            ...
            <div className="border-border/60 border-y py-8">
              {DemoComponent ? (
                ...
                <Alert variant="destructive">
                  ...
                  <AlertDescription>The selected demo could not be loaded.</AlertDescription>
                </Alert>
              )}
            </div>
          </TabsContent>
          <TabsContent value="code">
            {DemoSource ? (
              ...
              <Alert variant="destructive">
                ...
                <AlertDescription>The selected demo source could not be loaded.</AlertDescription>
              </Alert>
            )}
          </TabsContent>
        </Tabs>
        ...
        <div className="space-y-2">
          ...
          <pre className="overflow-x-auto rounded-lg bg-muted p-4 text-sm">
            <code>bunx shadcn@latest add @protoform/{selectedDemo.registryName}</code>
          </pre>
        </div>
      </section>
    </div>
