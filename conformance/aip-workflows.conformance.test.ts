...
async function issues(schema: DescMessage, values: Record<string, unknown>) {
  return (await createProtoFormSchema(schema)["~standard"].validate(values))
    .issues;
}
...
describe("AIP workflow form conformance", () => {
  it("models AIP-152 Job configuration and Run request forms", () => {
    ...
    expect(
      parseProtoSchema(CreateWriteBookJobRequestSchema).fields.map(
        (candidate) => candidate.key
      )
    ).toEqual(["parent", "writeBookJob", "writeBookJobId"]);
    expect(parseProtoSchema(RunWriteBookJobRequestSchema).fields).toMatchObject(
  ...
  it("models AIP-153 import and export source choices plus partial failures", async () => {
    ...
    expect(field(importSchema, "source").schema?.map(({ key }) => key)).toEqual(
      ["cloudStorageSource", "inlineSource"]
    );
    expect(
      field(exportSchema, "destination").schema?.map(({ key }) => key)
    ).toEqual(["cloudStorageDestination", "archiveDestination"]);
    expect(
      getProtoFieldCustomData(
        field(parseProtoSchema(ImportBooksMetadataSchema), "partialFailures")
      )?.hidden
    ).toBe(true);
    expect(
  ...
  it("enforces AIP-159 cross-collection and AIP-217 partial-result controls", async () => {
    expect(
      parseProtoSchema(ListBooksRequestSchema).fields.map(({ key }) => key)
    ).toEqual([
      "parent",
      "pageSize",
      "pageToken",
    ...
    expect(
      ...
      expect.arrayContaining([
        expect.objectContaining({
          message:
            "partial success is available only when reading across collections",
        }),
      ])
    );

    expect(
      getProtoFieldBehaviors(ListBooksResponseSchema.field.unreachable)
    ).toContain(FieldBehavior.UNORDERED_LIST);
    expect(
      getProtoPartialResult(
  ...
  it("models the AIP-165 purge preview and destructive confirmation", async () => {
    ...
    expect(
      await issues(PurgeBooksRequestSchema, { parent: "publishers/acme" })
    ).toBeDefined();
    expect(
      getProtoPurgePlan(
  ...
  it("models AIP-236 policy experiments and distinguishes preview from commit", async () => {
    ...
    expect(
      getProtoFieldCustomData(field(experiment, "previewMetadata"))?.hidden
    ).toBe(true);
    ...
    for (const schema of [
      ...
      expect(parseProtoSchema(schema).fields).toMatchObject([
        { key: "name", required: true },
      ]);
    }
    expect(
      parseProtoSchema(CommitPolicyExperimentRequestSchema).fields
    ).toMatchObject([
      { key: "name", required: true },
