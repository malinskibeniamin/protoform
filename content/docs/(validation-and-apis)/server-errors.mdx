---
title: Server errors
description: "Preserve structured Connect and gRPC errors from the API to accessible form feedback. See the practical Protoform implementation guide."
---
...
Use `serverPathPrefixes` when equivalent server contracts may wrap the same message under different
request fields:
...
Both `spec.config.host` and `instance.config.host` then map to `config.host`. The singular and plural
options combine: `serverPathPrefix` is checked first, followed by `serverPathPrefixes` in array order.
...
Protoform strips the first exact matching prefix, walks the protobuf descriptor, converts protobuf `snake_case` names
