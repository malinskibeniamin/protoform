...
description: Give coding agents typed Protoform context, discover focused form examples through MCP, and review generated protobuf migrations safely.
---
...
Use `/llms.txt` for the compact documentation map, `/llms-full.txt` for the complete corpus, or
connect an MCP client to `/mcp`. MCP clients should call `search_docs` first, then pass the returned
route directly to `get_page`. Focused demo pages return the same copyable source shown in their
