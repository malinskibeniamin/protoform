...
description: >-
  ...
  zwrotne formularza. Poznaj praktyczne użycie w Protoform.
---
...
Użyj `serverPathPrefixes`, gdy równoważne kontrakty serwera mogą opakować ten sam komunikat w różnych
polach żądania:
...
Zarówno `spec.config.host`, jak i `instance.config.host` zostaną wtedy zmapowane na `config.host`.
Opcje pojedyncza i mnoga są łączone: najpierw sprawdzany jest `serverPathPrefix`, a następnie elementy
`serverPathPrefixes` w kolejności tablicy.
...
Protoform usuwa pierwszy dokładnie pasujący prefiks, przechodzi przez deskryptor protobuf, konwertuje nazwy protobuf w formacie `snake_case`
