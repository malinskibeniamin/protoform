---
title: 伺服器錯誤
description: 將 API 的結構化 Connect 與 gRPC 錯誤轉換為無障礙表單回饋，並保留欄位錯誤、錯誤摘要與診斷內容。瞭解 Protoform 如何依據標準狀態碼與錯誤詳細資料，建立可復原、可診斷的完整表單體驗，避免依賴容易變動的訊息文字。
---
...
當等效的伺服器合約可能將相同訊息包裝在不同的請求欄位下時，請使用 `serverPathPrefixes`：
...
此時，`spec.config.host` 與 `instance.config.host` 都會對應至 `config.host`。單數與複數選項會合併：先檢查 `serverPathPrefix`，再依陣列順序檢查 `serverPathPrefixes`。
...
Protoform 會移除第一個完全相符的前綴、巡覽 protobuf descriptor、將 protobuf `snake_case` 名稱轉換為 TypeScript `camelCase`，並將 oneof 分支扁平化至 `{oneofLocalName}.value`。它會聚焦第一個已對應的欄位，並使用 React Hook Form 錯誤類型 `server` 加入所有已對應的違規項目。通用 Protovalidate 說明會使用與用戶端驗證相同且可採取行動的文案，而空白說明則會改用 `Review this value and try again.`。
