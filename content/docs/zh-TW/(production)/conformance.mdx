---
title: 一致性測試套件
description: "描述元解析、驗證、表單路徑及轉換的可執行合約。本指南進一步說明如何在 Protoform 中實際套用此功能，並涵蓋契約設計、表單產生、欄位驗證、錯誤處理、遷移策略、無障礙體驗、測試方法與正式環境整合，協助團隊採用經過驗證且可維護的實作路徑。"
---
...
此設計採用 [Protocol Buffers conformance suite](https://github.com/protocolbuffers/protobuf/tree/main/conformance) 中實用的職責分離方式：runner 管理案例與預期結果，實作則透過小型公開介面接受測試。由於目前支援的實作是 TypeScript，Protoform 使用 Vitest 在行程內執行此模型。若沒有其他語言介接器需要測試，加入子行程有線協定只會增加成本。
