---
title: AutoForm 註解
description: "從 protobuf schema 中繼資料產生具無障礙支援的 shadcn 表單。本指南進一步說明如何在 Protoform 中實際套用此功能，並涵蓋契約設計、表單產生、欄位驗證、錯誤處理、遷移策略、無障礙體驗、測試方法與正式環境整合。"
---
AutoForm 會讀取 protobuf descriptor 與選用的 UI 註解來產生欄位。
適用於管理介面、設定流程、connector 設定，以及需要隨 proto schema 演進的表單。

預設的 `auto-form` registry 項目使用 React Hook Form。安裝 `auto-form-tanstack` 並從
