...
describe("bookstore flagship demo", () => {
  it("lists, gets, and creates a book through a CEL-validated RHF stepper", async () => {
    ...
    await user.type(isbn, "9783161484101");
    await user.click(screen.getByRole("button", { name: "Continue" }));
    expect(await screen.findByText("ISBN must have a valid ISBN-13 check digit.")).toBeVisible();
    ...
    await user.type(bookId, "protocol-design");
    await user.type(screen.getByRole("textbox", { name: "Note" }), "Created through the generated binding.");
    await user.click(screen.getByRole("button", { name: "Create book" }));
  ...
  it("updates with an etag and field mask, then deletes with AutoForm", async () => {
    ...
    await user.click(await screen.findByRole("button", { name: "Domain-Driven Design" }));
    ...
    expect(screen.queryByRole("button", { name: "Domain Modeling" })).not.toBeInTheDocument();
  });
});
