...
describe('AutoForm stepper', () => {
  it('renders a vertical progress rail through the stepper config', () => {
    const schema = createMockProvider([
      { key: 'name', type: 'string', required: true, hints: { step: 'basics' } },
      { key: 'region', type: 'string', required: true, hints: { step: 'delivery' } },
      { key: 'approval', type: 'string', required: true, hints: { step: 'review' } },
    ]);
  ...
  it('renders one step at a time with semantic progress and linear navigation', async () => {
    ...
    expect(screen.getByRole('heading', { name: 'Basics' }).closest('section')).toHaveAttribute(
      'data-layout',
      'split'
    );
  ...
  it('shows the summary only on the final review step', async () => {
    ...
    const schema = createMockProvider(
      ...
      { name: 'Ada', region: 'eu-central', approval: 'OPS-42' }
    );
  ...
  it('blocks Continue on current-step errors without exposing later-step errors', async () => {
    ...
    const schema = createMockProvider(
      ...
      (values) => ({
        success: false,
        errors: [
          ...(!values.name ? [{ path: ['name'], message: 'Enter a name.' }] : []),
          ...(!values.region ? [{ path: ['region'], message: 'Choose a region.' }] : []),
        ],
      })
    );
  ...
  it('completes validation recovery and every step using only the keyboard', async () => {
    ...
    const schema = createMockProvider(
      ...
      {},
      (values) => ({
        success: Boolean(values.name && values.region && values.approval),
        ...(values.name && values.region && values.approval
          ? { data: values }
          : {
              errors: [
                ...(!values.name ? [{ path: ['name'], message: 'Enter a name.' }] : []),
                ...(!values.region ? [{ path: ['region'], message: 'Choose a region.' }] : []),
                ...(!values.approval ? [{ path: ['approval'], message: 'Enter approval.' }] : []),
              ],
            }),
      })
    );
