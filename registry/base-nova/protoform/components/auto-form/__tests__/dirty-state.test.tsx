...
describe.each([
  ...
  it('tracks nested, array, map, and oneof changes', async () => {
    ...
    async function expectEditThenClean(label: string, edit: () => void | Promise<void>) {
      await edit();
      await waitFor(() =>
        expect(onDirtyChange.mock.calls.at(-1)?.[0], `${label} should become dirty`).toBe(true)
      );
      await user.click(screen.getByRole('button', { name: 'Submit' }));
      await waitFor(() => expect(onDirtyChange).toHaveBeenLastCalledWith(false));
    }
    ...
    await waitFor(() => expect(onDirtyChange).toHaveBeenLastCalledWith(false));
    await expectEditThenClean('nested field', () =>
      user.type(screen.getByRole('textbox', { name: 'City' }), 'Warsaw')
    );
    await expectEditThenClean('array field', () =>
      user.click(screen.getByRole('button', { name: 'Add Tags' }))
    );
    await expectEditThenClean('map field', () =>
      user.click(screen.getByRole('button', { name: 'Add pair' }))
    );
