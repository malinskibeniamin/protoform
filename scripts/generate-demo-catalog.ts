...
import { type DemoCatalogEntry, demoCatalog } from "../examples/catalog/demo-catalog.js";
...
const registryDemoDirectory = join(root, "registry/base-nova/protoform/demo/catalog");
const registryRuntimeDirectory = join(root, "registry/base-nova/protoform/demo/runtime");

const generatedHeader = "{/* Generated by scripts/generate-demo-catalog.ts. */}";
const generatedSourceHeader = "// Generated by scripts/generate-demo-catalog.ts.";
...
function removeGeneratedFiles(directory: string, extension: string): void {
  ...
  for (const file of readdirSync(directory)) {
    ...
    if (file.endsWith(extension) && readFileSync(path, "utf8").includes("generate-demo-catalog.ts")) {
      unlinkSync(path);
    }
  }
}
...
function seoDescription(description: string): string {
  ...
  if (!expanded) {
    throw new Error(`Could not create a 110–160 character SEO description from: ${description}`);
  }
...
function generateDocs(): void {
  ...
  write(join(aipDirectory, "aip-example-catalog.mdx"), hubPage(aipHub.title, aipHub.description, aipHub.category));
  write(join(aipDirectory, "meta.ts"), meta("AIP examples", ["aip-example-catalog"]));
  ...
  for (const [category, title] of featureCategories) {
    ...
    write(join(directory, `${hub.slug}.mdx`), hubPage(hub.title, hub.description, hub.category));
...
function generateRegistryRuntime(): string[] {
  ...
  const schemaKeys = [...new Set(demoCatalog.map((demo) => demo.schemaKey))].toSorted();
  const schemaType = `export type DemoSchemaKey =\n${schemaKeys
    .map((key) => `  | ${JSON.stringify(key)}`)
    .join("\n")};`;
  const schemaSource = readFileSync(join(root, "examples/catalog/demo-schemas.ts"), "utf8")
    .replaceAll("../../conformance/gen/protoform/conformance/v1/", "./gen/protoform/conformance/v1/")
    .replace('import type { DemoSchemaKey } from "./demo-catalog.js";', schemaType);
...
function generateRegistry(): void {
  ...
  for (const demo of demoCatalog) {
    write(join(registryDemoDirectory, `${demo.slug}.tsx`), registryWrapper(demo));
  }
  ...
  const generatedNames = new Set(["protoform-demo-runtime", ...demoCatalog.map((demo) => demo.registryName)]);
  ...
  items.push({
    ...
    description: "Shared fixture descriptors and generated compatibility contracts for focused Protoform demos.",
